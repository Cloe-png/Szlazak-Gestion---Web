        ...
        <div id="listContainer" class="events-list-card animate__animated animate__fadeIn animate__delay-2s" style="display: none;">
            ...
            <div id="eventsList">
                @forelse($evenements as $evenement)
                    <div class="event-item {{ $evenement->type ?? 'chantier' }}" 
                         data-id="{{ $evenement->id }}"
                         data-type="{{ $evenement->type ?? 'chantier' }}"
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: [
                    ...
                    @foreach($evenements as $event)
                    {
                        ...
                        className: 'fc-event-{{ $event->type ?? "chantier" }}',
                        backgroundColor: getEventColor('{{ $event->type ?? "chantier" }}'),
                        borderColor: getEventBorderColor('{{ $event->type ?? "chantier" }}'),
                        textColor: 'white'
                    },
                    @endforeach
                ],
