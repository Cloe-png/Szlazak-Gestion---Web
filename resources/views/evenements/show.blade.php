...
<head>
    ...
    <style>
        :root {
            ...
            --fc-event-maintenance: #e74c3c;
            --fc-event-livraison: #f39c12;
            --fc-event-inspection: #9b59b6;
            --fc-event-autre: #95a5a6;
        }
        ...
        .type-chantier { background-color: rgba(52, 152, 219, 0.1); color: #3498db; }
        .type-reunion { background-color: rgba(46, 204, 113, 0.1); color: #2ecc71; }
        .type-maintenance { background-color: rgba(231, 76, 60, 0.1); color: #e74c3c; }
        .type-livraison { background-color: rgba(243, 156, 18, 0.1); color: #f39c12; }
        .type-inspection { background-color: rgba(155, 89, 182, 0.1); color: #9b59b6; }
        .type-autre { background-color: rgba(149, 165, 166, 0.1); color: #95a5a6; }
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <div class="d-flex align-items-center mt-2" style="opacity: 0.9;">
                        <span class="event-type-badge type-{{ $evenement->type ?? 'autre' }}">
                            <i class="fas fa-{{ $evenement->type == 'chantier' ? 'project-diagram' : ($evenement->type == 'reunion' ? 'users' : ($evenement->type == 'maintenance' ? 'tools' : ($evenement->type == 'livraison' ? 'truck' : ($evenement->type == 'inspection' ? 'search' : 'calendar'))) ) }} me-2"></i>
                            {{ ucfirst($evenement->type ?? 'Événement') }}
                        </span>
                        <span class="ms-3 status-badge status-{{ strtolower(str_replace(' ', '-', $evenement->statut)) }}">
                            {{ $evenement->statut }}
                        </span>
