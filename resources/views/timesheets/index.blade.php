        ...
        <nav class="navbar navbar-expand-lg navbar-light container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="{{ route('timesheets.index') }}">
                            ...
                            <tbody>
                                @foreach($groupedByUser as $userId => $userTimesheets)
                                        ...
                                        <td colspan="8">
                                            <div class="personne-info">
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted ms-2">{{ $user->role->nom }}</small>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    @foreach($groupedByWeek as $weekKey => $weekTimesheets)
                                        ...
                                        <tr class="semaine-row">
                                            <td>
                                                ...
                                                Semaine {{ $weekNumber }}
                                                <span class="week-badge">{{ $dateDebut->format('d/m') }} - {{ $dateFin->format('d/m') }}</span>
                                            </td>
                                            <td colspan="6"></td>
                                            <td></td>
                                        </tr>
                                        ...
                                        <!-- Détails des jours -->
                                        @foreach($weekTimesheets as $timesheet)
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <i class="fas fa-calendar-day text-muted me-2"></i>
                                                    <div>
                                                        <strong>{{ \Carbon\Carbon::parse($timesheet->date_travail)->format('d/m/Y') }}</strong>
                                                        <div class="small text-muted">{{ $timesheet->jour }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                {{ $timesheet->chantier->nom }}
                                                <div class="small text-muted">{{ $timesheet->chantier->statut }}</div>
                                            </td>
                                            <td>
                                                @if($timesheet->zone)
                                                    <span class="badge badge-zone">Zone {{ $timesheet->zone }}</span>
                                                @else
                                                    <span class="text-muted">-</span>
                                                @endif
                                            </td>
                                            <td>
                                                {{ \Carbon\Carbon::parse($timesheet->heure_debut)->format('H:i') }}<br>
                                                <small class="text-muted">{{ \Carbon\Carbon::parse($timesheet->heure_fin)->format('H:i') }}</small>
                                            </td>
                                            <td>
                                                @if($timesheet->pause)
                                            ...
                                            <td>
                                                @if($timesheet->heures_supp > 0)
                                                <span class="text-warning fw-bold">{{ $timesheet->heures_supp }}h</span>
                                                @else
                                                <span class="text-muted">0h</span>
                                                @endif
                                            </td>
                                            <td>
                                                <span class="badge badge-heures">{{ $timesheet->total_heures }}h</span>
                                            </td>
                                            <td>
                                                <div class="btn-group btn-group-sm">
                                                    <a href="{{ route('timesheets.edit', $timesheet) }}" class="btn btn-outline-warning btn-sm">
                                        ...
                                        <tr class="total-row">
                                            <td colspan="5" class="text-end">
                                                <small>Total semaine {{ $weekNumber }} :</small>
                                            </td>
                                            <td>
                                                <small class="text-warning fw-bold">{{ $totalHeuresSuppSemaine }}h</small>
                                            </td>
                                            <td>
                                                <span class="badge badge-heures badge-success">{{ $totalSemaine }}h</span>
                                            </td>
                                            <td></td>
                                        </tr>
                                    @endforeach
                                    ...
                                    <tr class="total-personne-row">
                                        <td colspan="5" class="text-end">
                                            <strong>Total {{ $user->nom }} :</strong>
                                        </td>
                                        <td>
                                            <strong class="text-warning">{{ $totalHeuresSuppPersonne }}h</strong>
                                        </td>
                                        <td>
                                            <span class="badge badge-heures badge-primary">{{ $totalPersonne }}h</span>
                                        </td>
                                        <td></td>
                                    </tr>
                                ...
                                <tr class="global-total-row">
                                    ...
                                    <td>
                                        <strong>{{ $globalTotalHeuresSupp }}h</strong>
                                    </td>
                                    <td>
                                        <span class="badge bg-light text-dark">{{ $globalTotalHeures }}h</span>
                                    </td>
                                    <td></td>
                                </tr>
                            </tbody>
